...
contract Xarians is DefaultOperatorFilterer, IERC721, Proxy, Functional {
    ...
    mapping(address => uint256) private _WLTracker;
    ...
    constructor() {
        _name = "Xarians";
        _symbol = "XaraC"; //citizen Contract.
        _baseURI = "https://bafybeif2knhguub2gu7e4gfbvdzijuqizajmg5zv66a27alc74x7sdjjpe.ipfs.nftstorage.link/";

        payoutWallet = 0x516e4BB39632690e674d36691264a6DcD7dF251E;
        
        totalTokens = 10000;
        price = 70 * (10 ** 15); // Replace leading value with price in finney
        whiteListMax = 11;
        maxPerWallet = 10000;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == Xarians.onERC721Received.selector;
    }
    ...
    function airDrop(address[] memory _to) external onlyOwner {
        ...
        for(uint256 i = 0; i < qty; i++) {
            ...
            numberMinted ++;  //increment here in case of gas overload
        }
    }

    function mint(uint256 qty) public payable reentryLock {
        require(mintActive, "Mint not open");
        require((numberMinted + qty) > numberMinted, "Math overflow error");
        require((numberMinted + qty) <= totalTokens, "Cannot fill order");
        require( qty + _mintTracker[_msgSender()] < maxPerWallet, "Wallet Limit Reached");
        require( msg.value == price * qty, "Wrong Eth Amount");
    ...
    function WLmint(uint256 qty) public payable reentryLock {
        require(whitelistActive, "Whitelist not open");
        require((numberMinted + qty) > numberMinted, "Math overflow error");
        require((numberMinted + qty) <= totalTokens, "Cannot fill order");
        require( qty + _WLTracker[_msgSender()] < whiteListMax, "Wallet Limit Reached");
        require( msg.value == price * qty, "Wrong Eth Amount");
        require( _whitelist[_msgSender()] == true, "Unauthorized Access");

        uint256 mintSeedValue = numberMinted;
        _WLTracker[_msgSender()] += qty;
        numberMinted += qty;
    ...
    /////////////////// Proxy Access for Staking /////////////////
    function proxyTransfer(address _from, address _to, uint256 _tokenId) external proxyAccess 	{
    ...
    function addWL(address wlAddress) public onlyOwner {
        _whitelist[wlAddress] = true;
    }

    function addManyWL(address[] memory wlAddress) public onlyOwner {
        for (uint256 i; i< wlAddress.length; i++){
            _whitelist[wlAddress[i]] = true;
        }
    }

    function setWLmax(uint256 wlMaxPerWallet) public onlyOwner {
        whiteListMax = wlMaxPerWallet;
    }
    ...
    function checkWhitelist(address walletAddress) public view returns (bool){
        return _whitelist[walletAddress];
    }
    ...
    function setPayoutWallet( address newWallet) public onlyOwner {
